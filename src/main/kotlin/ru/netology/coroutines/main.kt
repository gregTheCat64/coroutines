        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
                        val authorsOfComments = getComments(client, post.id).map {
                            AuthorsOfComment(author = getAuthor(client, it.authorId))
                        }
                        async {
...
suspend fun getCommentAuthor(client: OkHttpClient, id: Long): Author =
    makeRequest("$BASE_URL/api/slow/authors/$id", client, object : TypeToken<Author>() {})
